...
hl.exec_cmd("dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP XDG_SESSION_TYPE DISPLAY DBUS_SESSION_BUS_ADDRESS")
...
hl.config({
    ...
    general = {
        ...
        col = {
            active_border   = { colors = {colors.highlight1, colors.highlight2}, angle = 45 },
            inactive_border = "#2a2538",
        },
...
-- Default springs
hl.curve("easy", { type = "spring", mass = 0.6, stiffness = 200, dampening = 15 })
...
hl.animation({ leaf = "windowsOut",    enabled = true,  speed = 1.8,  bezier = "linear",       style = "popin 87%" })
...
hl.animation({ leaf = "workspacesOut", enabled = true,  speed = 1.8,  bezier = "almostLinear", style = "fade" })
...
hl.window_rule({
    name  = "vscode_transparency",
    match = {
        class = "code",
    },
    opacity = "0.9 0.9 override 0.9 override"
})
...
("playerctl play-pause"), { locked = true })
...
hl.bind(mainMod .. " + F", hl.dsp.window.fullscreen_state({ internal = 1, client = 0, action = "toggle", layout_aware = true}))

-- Trigger when the lid closes (Switch turns ON) -> Disable eDP-1 using your Lua hook
hl.bind("switch:on:Lid Switch", function()
hl.monitor({ output = 'eDP-2', disabled = true })
end, { locked = true })

-- Trigger when the lid opens (Switch turns OFF) -> Re-enable eDP-1 using your Lua hook
